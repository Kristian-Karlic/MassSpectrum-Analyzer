...
class TableEditorDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        if self.data_type == "modifications":
            ...
            self.table.setHorizontalHeaderLabels(
                ["Base Ion", "Series Name", "Mass Offset", "Color", "Restriction"]
            )
            ...
            self.base_ion_delegate = BaseIonComboDelegate(
                ["b", "y", "a", "c", "x", "z", "z+1", "c-1", "MH", "d", "w", "v"], self
            )
        ...
        self.duplicate_row_btn.setStyleSheet(
    ...
    def add_row(self):
        ...
        if self.data_type == "modifications":
            ...
            self.table.setItem(
                row_count, 3, QTableWidgetItem("#000000")
            )  # Color (black)
        elif self.data_type == "custom_ion_series":
            self.table.setItem(
                row_count, 0, QTableWidgetItem("b")
            )  # Base Ion (default to b)
            self.table.setItem(
                row_count, 1, QTableWidgetItem("New Series")
            )  # Series Name
        ...
        try:
            ...
            if reply == QMessageBox.StandardButton.Cancel:
                ...
                self.current_df = pd.concat(
                    [self.current_df, imported_df], ignore_index=True
                )
    ...
    def export_csv(self):
        ...
        if data.empty:
            QMessageBox.information(
                self, "No Data", "No data to export.", QMessageBox.StandardButton.Ok
            )
            return
        ...
        for row in range(self.table.rowCount()):
            ...
            if has_data:  # Only include rows with data
                ...
                if self.data_type == "diagnostic_ions" and len(row_data) >= 4:
                    if (
                        not row_data[1] and row_data[0]
                    ):  # HTML Name empty but Name exists
    ...
    def save_changes(self):
        ...
        try:
            ...
            if (
                self.data_type != "custom_ion_series"
            ):  # Don't check duplicates for custom ion series
                name_column = None
            ...
            if (
                self.data_type == "custom_ion_series"
                and "Series Name" in updated_df.columns
            ):
                reserved_chars = {"*", "~", "^"}
                bad_names = []
