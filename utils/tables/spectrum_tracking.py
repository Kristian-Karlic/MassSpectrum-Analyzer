...
class SpectrumTrackerWidget(QWidget):
    ...
    def _init_dataframes(self):
        ...
        export_columns = [
            ...
            "Glycan Max Charge",
            "Glycan SNFG",
            "Enable Labile Losses",
            "Enable Remainder Ions",
            "Enable Mod Neutral Losses",
    ...
    def _add_to_export_list_with_quality(self, quality):
        ...
        fragmented_bonds = (
            self.current_settings_data.get("fragmented_bonds", "")
            if self.current_settings_data
            else ""
        )
        annotated_tic = (
            self.current_settings_data.get("annotated_tic", "")
            if self.current_settings_data
            else ""
        )
        ...
        tracking_fields = {
            ...
            "Quality": quality,
            "User Modifications": (
                str(self.current_settings_data.get("user_modifications", []))
                if self.current_settings_data
                else ""
            ),
            "PPM Tolerance": (
                self.current_settings_data.get("ppm_tolerance", "")
                if self.current_settings_data
                else ""
            ),
            "Text Annotation Threshold": (
                self.current_settings_data.get("text_annotation_threshold", "")
                if self.current_settings_data
                else ""
            ),
            "Max Neutral Losses": (
                self.current_settings_data.get("max_neutral_losses", "")
                if self.current_settings_data
                else ""
            ),
            ...
            # Annotation feature flags
            "Glycan Y Enabled": (
                self.current_settings_data.get("glycan_y_enabled", False)
                if self.current_settings_data
                else False
            ),
            "Glycan Composition": (
                self.current_settings_data.get("glycan_composition_str", "")
                if self.current_settings_data
                else ""
            ),
            "Glycan Max Charge": (
                self.current_settings_data.get("glycan_max_charge", "")
                if self.current_settings_data
                else ""
            ),
            "Glycan SNFG": (
                self.current_settings_data.get("glycan_snfg_enabled", False)
                if self.current_settings_data
                else False
            ),
            "Enable Labile Losses": (
                self.current_settings_data.get("enable_labile_losses", False)
                if self.current_settings_data
                else False
            ),
            "Enable Remainder Ions": (
                self.current_settings_data.get("enable_remainder_ions", False)
                if self.current_settings_data
                else False
            ),
            "Enable Mod Neutral Losses": (
                self.current_settings_data.get("enable_mod_nl", False)
                if self.current_settings_data
                else False
            ),
            # Store all ion type selections in separate columns
            "Selected Ion Types": (
                str(self.current_settings_data.get("selected_basic_ions", []))
                if self.current_settings_data
                else ""
            ),
            "Selected Neutral Loss Types": (
                str(self.current_settings_data.get("selected_neutral_loss_ions", []))
                if self.current_settings_data
                else ""
            ),
            "Selected Internal Ion Types": (
                str(self.current_settings_data.get("selected_internal_ions", []))
                if self.current_settings_data
                else ""
            ),
            "Selected Custom Ions": (
                str(self.current_settings_data.get("selected_custom_ions_data", []))
                if self.current_settings_data
                else ""
            ),
            "Selected Diagnostic Ions": (
                str(self.current_settings_data.get("selected_diagnostic_ions_data", []))
                if self.current_settings_data
                else ""
            ),
        }
        ...
        logger.debug(
            f"[DEBUG] Export list record ({quality}): {len(new_record)} fields"
        )

        # Proper DataFrame concatenation
        new_df = pd.DataFrame([new_record])
        self.export_list_df = pd.concat(
            [self.export_list_df, new_df], ignore_index=True
        )
    ...
    def _export_spectra(self):
        """Export spectra from the export list."""
        ...
        pdf_radio = QRadioButton("Individual PDF files + Excel data")
        ...
        try:
            if export_type == 0:
                ...
                pdf_count, excel_file = self._export_pdf_and_excel(
                    folder_path, main_window
                )
                QMessageBox.information(
                    self,
                    "Export Complete",
    ...
    def _generate_fragmentation_for_row(
        self,
        main_window,
        peptide,
        modifications,
        ...
        try:
            ...
            neutral_loss_ions = self._parse_stored_list(
                row.get("Selected Neutral Loss Types", "[]")
            )
            internal_ions = self._parse_stored_list(
                row.get("Selected Internal Ion Types", "[]")
            )

            # ENHANCED: Parse stored custom and diagnostic ion data
            custom_ions_data = self._parse_stored_list(
                row.get("Selected Custom Ions", "[]")
            )
            diagnostic_ions_data = self._parse_stored_list(
                row.get("Selected Diagnostic Ions", "[]")
            )

            logger.debug("[DEBUG] Parsed from stored data:")
            ...
            logger.debug("[DEBUG] Generating fragmentation with:")
            ...
            logger.debug(
                f"[DEBUG] Fragmentation successful: {len(matched_data)} matched, {len(theoretical_data)} theoretical"
            )
    ...
    def _parse_stored_list(self, stored_string):
        ...
        try:
            ...
            logger.debug(
                f"[WARNING] Failed to parse stored list: {stored_string}, error: {e}"
            )
            return []
    ...
    def _get_spectral_data_for_scan(self, main_window, spectrum_file, scan_number):
        ...
        try:
            ...
            # Clean scan number
            scan_str = (
                scan_number.replace(".0", "")
                if scan_number.endswith(".0")
                else scan_number
            )
    ...
    def _create_temporary_export_dialog(self, main_window):
        ...
        try:
            ...
            main_viewer = None
            if (
                hasattr(main_window, "annotation_tab_manager")
                and main_window.annotation_tab_manager.mass_spec_viewer
            ):
            ...
            logger.debug("[DEBUG] Creating temporary export dialog...")
            ...
            dialog.setFixedSize(
                main_size.width() + 20, main_size.height() + 50
            )  # Add padding for title bar
            ...
            if hasattr(main_viewer, "available_modifications"):
                export_viewer.set_available_modifications(
                    main_viewer.available_modifications
                )
            ...
            logger.debug(
                f"[DEBUG] Temporary export dialog created with size: {dialog.size()}"
            )
    ...
    def _generate_pdf_filename(self, row, idx):
        """Generate PDF filename for batch export."""
        ...
        filename = f"{idx:03d}_{base_filename}.pdf"
        filename = (
            filename.replace(" ", "_")
            .replace("/", "_")
            .replace("\\", "_")
    ...
    def _generate_graph_for_row_in_export_viewer(self, export_viewer, row, main_window):
        ...
        try:
            ...
            if not peptide or peptide == "Unknown":
                logger.debug("[WARNING] No valid peptide for row")
                return False
            ...
            mods_to_use = (
                user_mods_str
                if user_mods_str and user_mods_str != "[]"
                else original_mods_str
            )

            try:
                modifications = (
                    ast.literal_eval(mods_to_use) if mods_to_use != "[]" else []
                )
            except Exception:
                modifications = []
            ...
            ppm_tolerance = (
                float(row.get("PPM Tolerance", 10)) if row.get("PPM Tolerance") else 10
            )
            max_neutral_losses = (
                int(row.get("Max Neutral Losses", 1))
                if row.get("Max Neutral Losses")
                else 1
            )

            # Get spectrum file path and scan number
            spectrum_file = str(
                row.get("spectrum_file_path", row.get("Spectrum file", ""))
            )
            scan_number = str(row.get("index", row.get("Scan", "")))

            logger.debug(
                f"[DEBUG] Generating graph in EXPORT DIALOG for: {peptide}, scan: {scan_number}"
            )

            # Extract spectral data for this scan
            spectral_data = self._get_spectral_data_for_scan(
                main_window, spectrum_file, scan_number
            )
            if not spectral_data:
                logger.debug(
                    f"[WARNING] No spectral data found for {spectrum_file}, scan {scan_number}"
                )
                return False
            ...
            logger.debug(
                f"[DEBUG] Setting data in export dialog viewer for {peptide}..."
            )
            ...
            row_data = dict(row)
            row_data["_export_dialog_id"] = (
                f"{peptide}_{scan_number}_{id(row)}"  # Make it truly unique
            )

            # Restore glycan settings on the export viewer so PDFs include glycan Y-ions
            export_viewer.glycan_y_enabled = (
                str(row.get("Glycan Y Enabled", "False")).lower() == "true"
            )
            export_viewer.glycan_composition_str = str(
                row.get("Glycan Composition", "")
            )
            try:
                export_viewer.glycan_max_charge = int(
                    float(row.get("Glycan Max Charge", 4))
                )
            except (ValueError, TypeError):
                export_viewer.glycan_max_charge = 4
            ...
            if not hasattr(export_viewer, "df") or export_viewer.df.empty:
                logger.debug("[WARNING] Export viewer data not set properly")
                return False

            # Verify peptide sequence was set
            if (
                hasattr(export_viewer, "peptide_sequence")
                and export_viewer.peptide_sequence != peptide
            ):
                logger.debug(
                    f"[WARNING] Peptide sequence not set correctly: expected {peptide}, got {export_viewer.peptide_sequence}"
                )
                return False

            logger.debug(
                f"[DEBUG] Export dialog viewer data set successfully: {len(export_viewer.df)} rows for {peptide}"
            )
            return True

        except Exception as e:
            logger.debug(
                f"[ERROR] Failed to generate graph for row in export dialog: {e}"
            )
            traceback.print_exc()
            return False

    def _export_pdf_and_excel(self, folder_path, main_window):
        """Export one vector PDF per spectrum + Excel data using the Qt-native PDF exporter."""
        total_rows = len(self.export_list_df)
        exported_count = 0

        progress = self._create_modal_progress_dialog("Exporting PDF Files", total_rows)
        progress.show()
        ...
        try:
            self._update_progress_percentage(
                progress, 0, total_rows, "Initializing PDF export"
            )
            export_dialog.show()
            QApplication.processEvents()

            columns = list(self.export_list_df.columns)
            col_idx = {col: pos for pos, col in enumerate(columns, start=1)}
            idx_peptide = col_idx.get("Peptide")
            idx_scan = col_idx.get("Scan")

            for row_pos, row_tuple in enumerate(
                self.export_list_df.itertuples(index=False, name=None)
            ):
                row = dict(zip(columns, row_tuple))
                current_item = row_pos + 1
                peptide = (
                    str(row_tuple[idx_peptide - 1])
                    if idx_peptide is not None
                    else "Unknown"
                )
                scan = (
                    str(row_tuple[idx_scan - 1]) if idx_scan is not None else "Unknown"
                )
                ...
                self._update_progress_percentage(
                    ...
                    current_item,
                    total_rows,
                    f"Processing {peptide} (scan {scan})",
                )
                try:
                    if self._generate_graph_for_row_in_export_viewer(
                        export_viewer, row, main_window
                    ):
                        self._update_progress_percentage(
                            progress, current_item, total_rows, f"Rendering {peptide}"
                        )
                        ...
                        self._update_progress_percentage(
                            ...
                            current_item,
                            total_rows,
                            f"Exporting PDF for {peptide}",
                        )
                        filename = self._generate_pdf_filename(row, row_pos)
                        filepath = os.path.join(folder_path, filename)

                        export_viewer._export_pdf_to_file(filepath)
                        exported_count += 1
                        logger.debug(
                            f"Exported PDF {exported_count}/{total_rows}: {filename}"
                        )
                    else:
                        logger.debug(f"Failed to generate graph for row {row_pos}")
                except Exception as e:
                    logger.debug(f"Failed to export PDF for row {row_pos}: {e}")
                    traceback.print_exc()
                    continue

            self._update_progress_percentage(
                progress, total_rows, total_rows, "Creating Excel file"
            )
        ...
        progress.setLabelText(
            f"Export complete! {exported_count} PDF files + Excel data - 100%"
        )
    ...
    def _create_export_metadata(self):
        ...
        accepted_count = (
            len(self.export_list_df[self.export_list_df["Quality"] == "Accepted"])
            if not self.export_list_df.empty
            and "Quality" in self.export_list_df.columns
            else 0
        )
        declined_count = (
            len(self.export_list_df[self.export_list_df["Quality"] == "Declined"])
            if not self.export_list_df.empty
            and "Quality" in self.export_list_df.columns
            else 0
        )
    ...
    def _update_progress_percentage(
        self, progress, current_item, total_items, current_status="Processing"
    ):
        ...
        status_text = (
            f"{current_status}... ({current_item}/{total_items}) - {percentage}%"
        )
