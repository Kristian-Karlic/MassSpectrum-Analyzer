...
class PeptideDisplayMixin:
    ...
    def calculate_responsive_parameters(self, sequence_length):
        ...
        if sequence_length <= self.max_aa_for_full_size:
            ...
            self.current_font_size = max(
                self.min_font_size, self.base_font_size - font_reduction
            )

            # Letter spacing decreases to fit within fixed width - CALCULATE FROM FIXED WIDTH
            available_space = self.fixed_width - (
                2 * 25
            )  # Use fixed margin, not current start_x
            self.letter_spacing = max(
                self.min_letter_spacing, available_space / (sequence_length - 1)
            )
    ...
    def _update_peptide_visual_parameters(self):
        ...
        line_thickness, line_offset = self.calculate_responsive_parameters(
            sequence_length
        )
    ...
    def _apply_text_formatting(
        self, text_item, amino_acid: str, color: str = None, scale: float = 1.0
    ):
    ...
    def highlight_peptide_sequence(self, fragment_sequence):
        ...
        if (
            not fragment_sequence
            or not hasattr(self, "peptide_sequence")
    ...
    def reset_peptide_highlighting(self):
        ...
        for item_data in self.amino_acid_items:
            ...
            text_color = EditorConstants.TEXT_COLOR()
            text_item.setHtml(
                f'<span style="font-weight:bold; color:{text_color}; font-size:{font_size}px;">{amino_acid}</span>'
            )
        self.highlight_items.clear()
    ...
    def _highlight_peptide_range(self, start_pos, end_pos):
        ...
        position_map = {
            item_data["position"]: item_data for item_data in self.amino_acid_items
        }
        ...
        for i in range(start_pos, end_pos + 1):
            ...
            if item_data:
                ...
                amino_acid = item_data["amino_acid"]
                text_item.setHtml(
                    f'<span style="font-weight:bold; color:#CC0000; font-size:{font_size}px;">{amino_acid}</span>'
                )
    ...
    def add_fragment_line(
        self, position: int, ion_type: str, color: str, y_offset: int
    ):
        ...
        vertical_line = pg.PlotDataItem(x=[x_pos, x_pos], y=[0, final_y], pen=pen)

        # Horizontal arm: C-terminal (positive y) extends RIGHT,
        # N-terminal (negative y) extends LEFT
        if y_offset > 0:
            horizontal_line = pg.PlotDataItem(
                x=[x_pos, x_pos + arm_length], y=[final_y, final_y], pen=pen
            )
        else:
            horizontal_line = pg.PlotDataItem(
                x=[x_pos - arm_length, x_pos], y=[final_y, final_y], pen=pen
            )
    ...
    def highlight_fragment_line(self, position: int, ion_type: str):
        ...
        highlight_width = (
            getattr(self, "current_line_thickness", 2) * 2
        )  # Double width for highlight
    ...
    def _calculate_fragment_position(self, data):
        ...
        try:
            ...
            sequence_length = (
                len(self.peptide) if hasattr(self, "peptide") and self.peptide else 0
            )
    ...
    def _iter_fragment_positions(self):
        ...
        columns = list(matched_fragments.columns)
        ...
        has_annotation_settings = bool(getattr(self, "annotation_display_settings", None))

        for row_tuple in matched_fragments.itertuples(index=False, name=None):
            base_type = (
            ...
            ion_number_str = (
                str(row_tuple[idx_ion_number]) if idx_ion_number is not None else ""
            )
            ...
            color = (
                ...
                else ion_offsets[base_type]["color_default"]
            )
