...
class AnnotationUndoMixin:
    ...
    def show_peak_context_menu(self, event):
        ...
        if not self.current_peak_data:
            logger.debug("No current peak data for context menu")
            return
        ...
        copy_action.triggered.connect(
            lambda: self.copy_mz_to_clipboard_with_data(menu_peak_data)
        )
        ...
        if not matching_rows.empty:
            ...
            is_matched = (
                matched_status != "No Match"
                and pd.notna(matched_status)
                and str(matched_status).strip() != ""
            )
        ...
        if is_matched:
            ...
            remove_annotation_action = QAction(
                "Remove Annotation (Make Unmatched)", self
            )
            remove_annotation_action.triggered.connect(
                lambda: self.remove_peak_annotation_with_data(menu_peak_data)
            )
            ...
            if (
                ...
                and "Alternative Matches" in matching_rows.columns
            ):
                    ...
                    if alt_list:
                        ...
                        for alt in alt_list:
                            ...
                            swap_action.triggered.connect(
                                lambda checked, ad=alt_data, mpd=menu_peak_data: self.swap_peak_to_alternative(
                                    mpd, ad
                                )
                            )
    ...
    def _reset_all_annotation_positions(self):
        """Clear all user-dragged label positions and replot to restore defaults."""
        if hasattr(self, "_saved_annotation_positions"):
            self._saved_annotation_positions.clear()
        # Also clear any active multi-selection
        if hasattr(self, "_selected_annotation_items"):
            from ..classes.interactivetext import EnhancedInteractiveTextItem
            for item in list(self._selected_annotation_items):
                try:
        ...
        if hasattr(self, "plot_spectrum"):
            # reset_positions=True skips re-saving current (dragged) positions
            self.plot_spectrum(reset_positions=True)
    ...
    def remove_peak_annotation_with_data(self, peak_data):
        ...
        """
        if not peak_data:
            logger.debug("No peak data for annotation removal")
            return
        ...
        primary_isotope = (
            int(primary_row.get("Isotope", 0))
            if pd.notna(primary_row.get("Isotope"))
            else 0
        )
        ...
        if primary_isotope == 0 and primary_ion_type and primary_ion_number is not None:
            ...
            isotope_candidates = self.df[
                ...
                & (
                    pd.to_numeric(self.df["Charge"], errors="coerce")
                    == int(primary_charge if pd.notna(primary_charge) else 1)
                )
        ...
        if removed_count > 1:
            logger.debug(
                f"Removed annotation for peak at m/z {target_mz} and {removed_count - 1} associated isotope(s)"
            )
        else:
    ...
    def swap_peak_to_alternative(self, peak_data, alt_ion_data):
        ...
        try:
            old_alt_list = (
                json.loads(old_alt_raw)
                if isinstance(old_alt_raw, str) and old_alt_raw
                else []
            )
        except (json.JSONDecodeError, TypeError):
            old_alt_list = []
        ...
        new_alt_list = []
        old_isotope = (
            int(old_row.get("Isotope", 0)) if pd.notna(old_row.get("Isotope")) else 0
        )
        if old_matched != "No Match" and pd.notna(old_matched) and old_isotope == 0:
            old_charge = (
                int(old_row["Charge"]) if pd.notna(old_row.get("Charge")) else 1
            )
            charge_str = f"{old_charge}+" if old_charge > 1 else ""
            old_label = f"{old_row['Ion Type']}{old_row['Ion Number']}{charge_str}"
            old_ppm = (
                float(old_row["error_ppm"])
                if pd.notna(old_row.get("error_ppm"))
                else 0.0
            )
            new_alt_list.append(
                {
        ...
        for existing_alt in old_alt_list:
            if (
                existing_alt.get("Ion Type") == alt_ion_data.get("Ion Type")
                and existing_alt.get("Ion Number") == alt_ion_data.get("Ion Number")
                and existing_alt.get("Charge") == alt_ion_data.get("Charge")
            ):
        ...
        new_row_data = pd.Series(
            {
                ...
                "Ion Series Type": alt_ion_data.get(
                    "Ion Series Type", "Standard-Ion-Series"
                ),
            }
        )
        ...
        if alt_isotope == 0:
            ...
            for i, (line_item, data) in enumerate(self.peak_lines):
                if abs(data["mz"] - target_mz) < tolerance:
                    new_pen = pg.mkPen(
                        alt_ion_data["Color"], width=PlotConstants.PEAK_LINE_WIDTH
                    )
        ...
        logger.debug(
            f"Swapped peak at m/z {target_mz:.4f} from {old_row.get('Ion Type', '?')}{old_row.get('Ion Number', '?')} to {alt_ion_data['Ion Type']}{alt_ion_data['Ion Number']}"
        )
    ...
    def copy_mz_to_clipboard_with_data(self, peak_data):
        ...
        if peak_data:
            ...
            logger.debug("No peak data available for copying")
    ...
    def undo_annotation_removal(self):
        ...
        for mz_val in all_mzs:
            ...
            if not current_rows.empty:
                current_frag_seq = (
                    current_rows.iloc[0].get("Fragment Sequence", "") or ""
                )
            ...
            for idx, data in original_data.items():
                ...
                if idx in self.df.index:
                    ...
                    if matched_df_mask.any():
                        for column, value in data.items():
                            if column in self.matched_df.columns:
                                self.matched_df.loc[matched_df_mask, column] = (
        ...
        if primary_data:
            primary_peak_data = next(iter(primary_data.values()), {})
            fragment_sequence = primary_peak_data.get("Fragment Sequence", "") or ""
            ...
            if isotope_num > 0 and idx in self.df.index:
                ...
                for i, (line_item, line_data) in enumerate(self.peak_lines):
                    if abs(line_data["mz"] - iso_mz) < tolerance:
                        new_pen = pg.mkPen(
                            iso_color, width=PlotConstants.PEAK_LINE_WIDTH
                        )
                        line_item.setPen(new_pen)
                        line_item.original_pen = new_pen
                        self.peak_lines[i] = (
    ...
    def restore_peak_annotation(self, target_mz, original_data, fragment_sequence=None):
        ...
        first_data = next(iter(original_data.values()), {})
        ...
        annotation_text = first_data.get(
            "text_annotation", ""
        )  # Unicode for plot display
        html_annotation_text = first_data.get(
            "html_annotation", ""
        )  # HTML for tooltips
        ...
        if isotope_numeric == 0 and annotation_text and annotation_text.strip():
            ...
            if is_rotated:
                vertical_offset = (
                    PlotConstants.PEAK_LABEL_OFFSET + 15
                )  # Extra offset for rotated text
            else:
            ...
            text_item._html_annotation = (
                html_annotation_text  # Store HTML version for tooltips
            )
        ...
        if (
            error_ppm is not None
            and not (isinstance(error_ppm, str) and error_ppm.strip() == "")
            and not pd.isna(error_ppm)
            and isotope_numeric == 0
        ):
            ...
            try:
                ...
                scatter_data = {
                    ...
                    "idx": next(iter(original_data), None),
                    "ion_type": first_data.get("Ion Type", ""),
                    "original_brush": original_color,
                    "scatter_item": scatter_item,
                    "annotation": html_annotation_text
                    or annotation_text,  # Use HTML for tooltips
            ...
            logger.debug("Not restoring error point - conditions not met")
    ...
    def update_undo_button_state(self):
        ...
        if hasattr(self, "undo_annotation_action"):
            ...
            if has_history:
                ...
                first_orig = next(iter(last_entry["original_data"].values()), {})
                if "Alternative Matches" in first_orig and first_orig.get(
                    "Alternative Matches"
                ):
            ...
            logger.debug("undo_annotation_action not found")
