...
class PersistentTooltip(QLabel):
    def __init__(self, parent=None):
        ...
        self.follow_timer.setInterval(
            PlotConstants.TOOLTIP_FOLLOW_INTERVAL_MS
        )  # Update every 50ms
    ...
    def _on_focus_changed(self, old_widget, new_widget):
        ...
        if (
            new_widget
            and not self.isAncestorOf(new_widget)
            and new_widget != self.parent()
        ):
            self.hide_tooltip()
    ...
    def show_tooltip(self, text, position):
        ...
        offset_position = position + QPoint(
            PlotConstants.TOOLTIP_OFFSET_X, PlotConstants.TOOLTIP_OFFSET_Y
        )
    ...
    def follow_cursor(self):
        ...
        if self.isVisible():
            ...
            offset_position = cursor_pos + QPoint(
                PlotConstants.TOOLTIP_OFFSET_X, PlotConstants.TOOLTIP_OFFSET_Y
            )
            self.move(offset_position)
