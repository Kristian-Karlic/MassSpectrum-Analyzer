...
class DataframeViewerDialog(QDialog):
    def __init__(
        ...
        self.original_matched_df = (
            matched_df.copy() if matched_df is not None else pd.DataFrame()
        )
        self.original_theoretical_df = (
            theoretical_df.copy() if theoretical_df is not None else pd.DataFrame()
        )
        self.original_details_df = (
            details_df.copy() if details_df is not None else pd.DataFrame()
        )
    ...
    def _setup_ui(self):
        ...
        if not self.original_theoretical_df.empty:
            self.theoretical_filter_checkbox = QCheckBox(
                "Show only Isotope = 0 (Theoretical)"
            )
        ...
        self.matched_table = self._create_table_for_dataframe(
            self.current_matched_df, "Matched Fragments"
        )
        self.tab_widget.addTab(self.matched_table, "Matched Fragments")

        if not self.original_theoretical_df.empty:
            self.theoretical_table = self._create_table_for_dataframe(
                self.current_theoretical_df, "Theoretical Fragments"
            )
    ...
    def _update_filter_states(self):
        ...
        elif current_tab == 1 and hasattr(
            self, "theoretical_filter_checkbox"
        ):  # Theoretical Fragments tab
    ...
    def _apply_initial_filters(self):
        ...
        if not self.original_theoretical_df.empty and hasattr(
            self, "theoretical_filter_checkbox"
        ):
    ...
    def _apply_filters(self):
        ...
        if (
            current_tab == 0
            and hasattr(self, "matched_filter_checkbox")
            and self.matched_filter_checkbox.isChecked()
        ):
        ...
        if (
            current_tab == 1
            and hasattr(self, "theoretical_filter_checkbox")
            and self.theoretical_filter_checkbox.isChecked()
            and not self.original_theoretical_df.empty
        ):
            self._filter_theoretical_data()
    ...
    def _filter_matched_data(self):
        ...
        if "Matched" in self.current_matched_df.columns:
            ...
            mask = (
                ...
                & (
                    pd.to_numeric(
                        self.current_matched_df["Matched"], errors="coerce"
                    ).notna()
                )
            )
    ...
    def _filter_theoretical_data(self):
        ...
        if "Isotope" in self.current_theoretical_df.columns:
            ...
            isotope_numeric = pd.to_numeric(
                self.current_theoretical_df["Isotope"], errors="coerce"
            )
    ...
    def _populate_tables(self):
        ...
        if (
            hasattr(self, "theoretical_table")
            and not self.original_theoretical_df.empty
        ):
            self._populate_table(self.theoretical_table, self.current_theoretical_df)
            ...
            for col in range(len(dataframe.columns)):
                ...
                if pd.isna(value):
                    ...
                    if isinstance(value, float):
                        item_text = (
                            f"{value:.6f}" if abs(value) < 1000 else f"{value:.2f}"
                        )
                    else:
                        item_text = str(value)
                else:
    ...
    def _create_table_for_dataframe(self, df, tab_name):
        ...
        if df.empty:
            ...
            for row_idx, row_tuple in enumerate(
                df_reordered.itertuples(index=False, name=None)
            ):
                for col_idx, value in enumerate(row_tuple):
                    ...
                    if pd.isna(value):
                        ...
                        display_value = (
                            f"{value:.4f}" if abs(value) < 1000 else f"{value:.2e}"
                        )
                    else:
