    ...
    def create_experiment_data(app_instance) -> dict:
        ...
        return {
            "metadata": {
                ...
                "total_psms": (
                    len(app_instance.merged_df)
                    if hasattr(app_instance, "merged_df")
                    else 0
                ),
                "unique_peptides": (
                    app_instance.merged_df["Peptide"].nunique()
                    if hasattr(app_instance, "merged_df")
                    and "Peptide" in app_instance.merged_df.columns
                    else 0
                ),
            ...
            "file_paths": {
                ...
                "df_file_paths": (
                    app_instance.df_file_paths.to_dict("records")
                    if not app_instance.df_file_paths.empty
                    else []
                ),
            },
            "settings": {
            ...
            "extracted_spectral_data": getattr(
                app_instance, "extracted_spectral_data", {}
            ),
            "merged_df": (
                app_instance.merged_df.to_dict("records")
                if hasattr(app_instance, "merged_df")
                else []
            ),
        }
