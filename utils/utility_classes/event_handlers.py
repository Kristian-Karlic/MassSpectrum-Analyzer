...
class EventHandlers:
    def __init__(self, main_app):
        ...
        self._manual_annotation_from_psm_selection_pending = False
    ...
    def _connect_input_signals(self):
        ...
        self.main_app.ppm_tolerance_input.valueChanged.connect(
            self.on_ppm_tolerance_changed
        )
        self.main_app.text_annotation_threshold.valueChanged.connect(
            self.on_text_threshold_changed
        )
        self.main_app.max_neutral_losses_input.valueChanged.connect(
            self.on_neutral_losses_changed
        )

        # Checkbox signals
        self.main_app.enable_direct_scan_checkbox.stateChanged.connect(
            self.on_direct_scan_toggle
        )
    ...
    def _connect_scan_selection_signals(self):
        ...
        if hasattr(self.main_app, "extract_scan_button"):
            self.main_app.extract_scan_button.clicked.connect(
                self.on_extract_scan_clicked
            )
    ...
    def on_peptide_changed(self):
        ...
        if (
            hasattr(self.main_app, "current_parsed_mods")
            and self.main_app.current_parsed_mods
        ):
            self.main_app.annotation_tab_manager.set_modifications(
                self.main_app.current_parsed_mods
            )
    ...
    def on_peptide_selected(
        ...
        self._manual_annotation_from_psm_selection_pending = True
        ...
        self.main_app._autoselect_glycan_from_parsed_mods(parsed_mods)
        ...
        logger.info(
            f"Auto-filled peptide={peptide}, charge={charge}, modifications={parsed_mods}"
        )
    ...
    def on_raw_data_extracted(self, mz_array, intensity_array):
        ...
        if (
            self._manual_annotation_from_psm_selection_pending
            and getattr(self.main_app, "annotation_mode", "auto") == "manual"
        ):
            self.perform_adaptive_update()
    ...
    def on_interactive_modifications_changed(self, modifications: list):
        ...
        if getattr(
            self.main_app.annotation_tab_manager.mass_spec_viewer,
            "_updating_data",
            False,
        ):
        ...
        self.main_app._autoselect_glycan_from_parsed_mods(modifications)
    ...
    def on_analysis_requested(self, matched_data):
        ...
        logger.debug(
            f"[DEBUG] Analysis requested with matched data type: {type(matched_data)}"
        )

        # Get the current matched data from annotation tab if not provided
        if matched_data is None or (
            hasattr(matched_data, "empty") and matched_data.empty
        ):
            logger.debug(
                "[DEBUG] No matched data provided, getting from annotation tab"
            )
            matched_data = (
                self.main_app.annotation_tab_manager.get_current_matched_data()
            )

        if matched_data is None or (
            hasattr(matched_data, "empty") and matched_data.empty
        ):
            logger.debug("[DEBUG] Still no matched data available")
        ...
        logger.debug(
            f"[DEBUG] Opening analysis dialog with {len(matched_data)} matched peaks"
        )
    ...
    def on_adaptive_fragmentation_finished(self, result):
        ...
        try:
            ...
            logger.info(
                f"[ADAPTIVE] Fragmentation completed - Matched: {len(matched_data)}, Theoretical: {len(theoretical_data)}"
            )
            ...
            if extra_rows:
                ...
                theoretical_data = pd.concat(
                    [theoretical_data, theoretical_custom], ignore_index=True
                )
            ...
            if self.main_app.annotation_tab_manager.peptide_info_widget:
                fragmented_bonds = self.main_app.annotation_tab_manager.peptide_info_widget.calculate_fragmented_bonds(
                    peptide, matched_data
                )
                annotated_tic = self.main_app.annotation_tab_manager.peptide_info_widget.calculate_annotated_percentage(
                    matched_data
                )
            ...
            if current_tab and current_tab.objectName() == "analysis_tab":
                logger.debug(
                    "[DEBUG] Analysis tab is active, forcing immediate analysis update"
                )
                # Small delay to ensure data is fully set before analysis

                QTimer.singleShot(
                    100, self.main_app.analysis_tab_manager.refresh_all_analysis
                )
    ...
    def on_extract_scan_clicked(self):
        ...
        if not scan_number.strip():
            UIHelpers.show_validation_error(
                self.main_app, "Input Error", "Please enter a scan number."
            )
            return

        # Extract the scan data
        result = self.main_app.experiment_data_manager.extract_scan_data(
            selected_file, scan_number
        )
        ...
        if result:
            ...
            logger.debug(
                f"[DEBUG] Successfully populated m/z table with {len(data_pairs)} data points"
            )
    ...
    def on_custom_ion_selected(self, selected_row_dict):
        ...
        logger.info(
            f"[EVENT] Custom ion selected: {selected_row_dict.get('Series Name', 'Unknown')}"
        )
        ...
        for existing in self.main_app.selected_custom_ions_data:
            if existing.get("Series Name") == selected_row_dict.get(
                "Series Name"
            ) and existing.get("Base Ion") == selected_row_dict.get("Base Ion"):
                self.main_app.show_toast_message(
                    f"'{selected_row_dict.get('Series Name')}' is already in the list."
                )
                return
        ...
        self.on_settings_changed()
        self.main_app.show_toast_message(
            f"Added '{ion_data['Series Name']}' to selected custom ions."
        )

    def on_diagnostic_ion_selected(self, selected_row_dict):
        """Handle diagnostic ion selection from dropdown"""
        logger.info(
            f"[EVENT] Diagnostic ion selected: {selected_row_dict.get('Name', 'Unknown')}"
        )
        ...
        for existing in self.main_app.selected_diagnostic_ions_data:
            if existing.get("Name") == selected_row_dict.get("Name"):
                self.main_app.show_toast_message(
                    f"'{selected_row_dict.get('Name')}' is already in the list."
                )
                return
        ...
        self.on_settings_changed()
        self.main_app.show_toast_message(
            f"Added '{ion_data['Name']}' to selected diagnostic ions."
        )
    ...
    def on_settings_changed(self):
        ...
        if getattr(self.main_app, "annotation_mode", "auto") == "manual":
            logger.info("[EVENT] Manual annotation mode — skipping auto update")
            return
    ...
    def perform_adaptive_update(self):
        ...
        logger.debug(
            f"[DEBUG] Starting fragmentation for peptide: {peptide} with {len(mz_data)} m/z values"
        )
    ...
    def _run_fragmentation_adaptive(self):
        ...
        peptide = self.main_app.peptide_input.text().strip()
        max_charge = self.main_app.max_charge_input.value()
        user_mz_values = TableUtils.extract_mz_intensity_from_table(
            self.main_app.mz_table
        )
        ...
        calculate_isotopes = getattr(self.main_app, "calculate_isotopes", False)
        isotope_max = getattr(self.main_app, "isotope_max", 4)
        ...
        diagnostic_ions = DataGatherer.gather_diagnostic_ions(
            self.main_app.selected_diagnostic_ions_data
        )
            ...
            if mod_neutral_losses and modifications:
                ...
                for mod_idx, (mod_mass, _pos) in enumerate(modifications):
                    ...
                    if nl_cfg is None:
                        continue
                    mod_name = (
                        central_db.find_by_mass(mod_mass)
                    ...
                    for nl_i, nl_mass in enumerate(nl_cfg.get("neutral_losses", [])):
                        ...
                        if key not in seen:
                            ...
                            nl_symbol_entries.append(
                                (sym, f"NL{nl_i + 1}", -nl_mass, mod_name)
                            )
                    if nl_cfg.get("labile_loss", False):
                        ...
                        if key not in seen:
                            seen.add(key)
                            nl_symbol_entries.append(
                                ("~", "Labile loss", -labile, mod_name)
                            )
                        for rm_i, rm_mass in enumerate(
                            nl_cfg.get("remainder_ions", [])
                        ):
                            if rm_mass <= 0:
                                continue
                            sym = _rm_tag(rm_i)
        ...
        # Read glycan Y-ion settings from the viewer
        _viewer = getattr(
            getattr(self.main_app, "annotation_tab_manager", None),
            "mass_spec_viewer",
            None,
        )
        glycan_y_enabled = getattr(_viewer, "glycan_y_enabled", False)
        glycan_composition_str = (
            getattr(_viewer, "glycan_composition_str", "") if glycan_y_enabled else ""
        )
        glycan_max_charge = (
            getattr(_viewer, "glycan_max_charge", 4) if glycan_y_enabled else None
        )
        if glycan_y_enabled and "GlycanY" not in selected_ions:
            selected_ions = list(selected_ions) + ["GlycanY"]
    ...
    def _get_modifications_from_table(self) -> list[tuple[float, int]]:
        ...
        if (
            hasattr(self.main_app, "current_interactive_mods")
            and self.main_app.current_interactive_mods
        ):
    ...
    def _create_diagnostic_ion_rows(self):
        ...
        extra_rows = []
        diag_df = self.main_app.diagnostic_ions
        if diag_df is None or diag_df.empty:
            return extra_rows

        col_idx = {col: pos for pos, col in enumerate(diag_df.columns, start=1)}
        idx_include = col_idx.get("Include")
        idx_name = col_idx.get("Name")
        idx_mass = col_idx.get("Mass")
        idx_color = col_idx.get("Color")

        for row_tuple in diag_df.itertuples(index=False, name=None):
            include = row_tuple[idx_include - 1] if idx_include is not None else False
            if bool(include):  # Only include checked ions
                ion_name = row_tuple[idx_name - 1] if idx_name is not None else ""
                mass_val = row_tuple[idx_mass - 1] if idx_mass is not None else 0
                color = row_tuple[idx_color - 1] if idx_color is not None else "black"
    ...
    def _update_spectrum_tracker(self):
        ...
        if (
            not hasattr(self.main_app, "selected_row_data")
            or not self.main_app.selected_row_data
        ):
            return
        ...
        if (
            self.main_app.annotation_tab_manager.peptide_info_widget
            and hasattr(
                self.main_app.annotation_tab_manager.peptide_info_widget,
                "current_matched_data",
            )
            and self.main_app.annotation_tab_manager.peptide_info_widget.current_matched_data
            is not None
        ):
            ...
            fragmented_bonds = peptide_widget.calculate_fragmented_bonds(
                peptide, matched_data
            )
        ...
        # Viewer reference for glycan state
        _viewer = getattr(
            getattr(self.main_app, "annotation_tab_manager", None),
            "mass_spec_viewer",
            None,
        ...
        def _cb(attr):
            cb = getattr(self.main_app, attr, None)
            return cb.isChecked() if cb is not None else False

        # Gather comprehensive settings data
        settings_data = {
            "user_modifications": getattr(
                self.main_app, "current_interactive_mods", []
            ),
            "ppm_tolerance": self.main_app.ppm_tolerance_input.value(),
            ...
            # Y-Glycan settings
            "glycan_y_enabled": getattr(_viewer, "glycan_y_enabled", False),
            "glycan_composition_str": getattr(_viewer, "glycan_composition_str", ""),
            "glycan_max_charge": getattr(_viewer, "glycan_max_charge", 4),
            "glycan_snfg_enabled": _cb("glycan_snfg_cb"),
            # Labile / remainder / mod-NL
            "enable_labile_losses": _cb("enable_labile_losses_cb"),
            "enable_remainder_ions": _cb("enable_remainder_ions_cb"),
            "enable_mod_nl": _cb("enable_mod_nl_cb"),
        }

        # Update spectrum tracker through annotation manager
        self.main_app.annotation_tab_manager.set_current_spectrum(
            self.main_app.selected_row_data, settings_data
        )
