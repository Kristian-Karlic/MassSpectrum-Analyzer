...
class DropZoneWidget(QListWidget):
    ...
    def _is_duplicate(self, peptide_data):
        ...
        for i in range(self.count()):
            ...
            if hasattr(item, "peptide_data") and isinstance(item.peptide_data, dict):
                ...
                if (
                    existing_scan == new_scan
                    and existing_spectrum_file == new_spectrum_file
                ):
                    logger.info(
                        f"Duplicate detected: scan={new_scan}, file={new_spectrum_file}"
                    )
                    return True
    ...
    def update_theme(self):
        ...
        if self.placeholder_item:
            self.placeholder_item.setForeground(
                QColor(EditorConstants.DISABLED_COLOR())
            )
