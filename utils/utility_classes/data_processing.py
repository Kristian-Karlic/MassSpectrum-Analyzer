...
class DataProcessingUtils:
    ...
    def filter_dataframe(
        merged_df, topN=999999, unique_pep=False, unique_mod=False, groupby_column=None
    ):
        """
        ...
        logger.debug(
            f"[Filter] Settings: topN={topN}, unique_pep={unique_pep}, unique_mod={unique_mod}, groupby_column={groupby_column}"
        )
        ...
        def filter_group(group_df):
            ...
            if unique_mod and "Modified Peptide" in group_df.columns:
                ...
                result = (
                    group_df.groupby("Modified Peptide", group_keys=False)
                    .apply(
                        lambda x: x.nlargest(
                            min(topN, len(x)), "Hyperscore", keep="first"
                        )
                    )
                ...
                result = (
                    group_df.groupby("Peptide", group_keys=False)
                    .apply(
                        lambda x: x.nlargest(
                            min(topN, len(x)), "Hyperscore", keep="first"
                        )
        ...
        try:
            if groupby_column:
                ...
                if isinstance(groupby_column, list):
                    ...
                    missing_cols = [
                        col for col in groupby_column if col not in merged_df.columns
                    ]
                    if missing_cols:
                        logger.warning(
                            f"Missing columns for grouping: {missing_cols} - applying filters without grouping"
                        )
                        result_df = filter_group(merged_df)
                    else:
                        logger.debug(
                            f"[Filter] Applying filters grouped by: {' -> '.join(groupby_column)}"
                        )
                        filtered_dfs = []
                        for group_values, group in merged_df.groupby(groupby_column):
                            group_desc = " -> ".join(
                                [
                                    f"{col}={val}"
                                    for col, val in zip(groupby_column, group_values)
                                ]
                            )
                            logger.debug(
                                f"[Filter] Processing {group_desc} ({len(group)} PSMs)"
                            )
                            filtered_group = filter_group(group)
                            logger.info(
                                f"  → After filtering: {len(filtered_group)} PSMs"
                            )
                    ...
                    logger.debug(
                        f"[Filter] Applying filters grouped by: {groupby_column}"
                    )
                    filtered_dfs = []
                    for group_value, group in merged_df.groupby(groupby_column):
                        logger.debug(
                            f"[Filter] Processing {groupby_column}='{group_value}' ({len(group)} PSMs)"
                        )
                        filtered_group = filter_group(group)
                    ...
                    logger.warning(
                        f"Column '{groupby_column}' not found - applying filters without grouping"
                    )
                    result_df = filter_group(merged_df)
            else:
            ...
            logger.debug(
                f"[Filter] Final result: Filtered from {len(merged_df)} to {len(result_df)} rows"
            )
            if len(merged_df) > 0:
                logger.debug(
                    f"[Filter] Reduction: {((len(merged_df) - len(result_df)) / len(merged_df) * 100):.1f}% removed"
                )
...
class IonTypeGenerator:
    ...
    @staticmethod
    def generate_dynamic_ion_types(
        normal_checkboxes: dict, neutral_checkboxes: dict, max_losses: int
    ) -> List[str]:
        ...
        single_loss_ions = [
            ion for ion, cb in neutral_checkboxes.items() if cb.isChecked()
        ]

        all_ion_types = base_ions + single_loss_ions
        seen_ion_types = set(all_ion_types)
                ...
                if "-" in single_loss:
                    ...
                    if base_ion in loss_capable_ions and loss_type in loss_types:
                        for count in range(2, max_losses + 1):
                            ...
                            if multiple_loss_ion not in seen_ion_types:
                                all_ion_types.append(multiple_loss_ion)
                                seen_ion_types.add(multiple_loss_ion)
