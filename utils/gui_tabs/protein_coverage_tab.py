...
class ProteinCoverageTable(QTableWidget):
    ...
    def populate_data(self, coverage_df):
        ...
        for row_idx, row in enumerate(coverage_df.itertuples(index=False)):
            # Protein
            item = QTableWidgetItem(str(getattr(row, "Protein", "")))
            self.setItem(row_idx, 0, item)

            # Description
            item = QTableWidgetItem(str(getattr(row, "Description", "")))
            self.setItem(row_idx, 1, item)

            # Length - use numeric sorting
            length = int(getattr(row, "Length", 0))
            item = NumericTableWidgetItem(str(length), length)
            self.setItem(row_idx, 2, item)

            # Coverage % - use numeric sorting
            coverage = float(getattr(row, "Coverage_Percent", 0.0))
...
class ProteinCoverageTabManager:
    ...
    def load_fasta_file(self):
        ...
        try:
            ...
            self.main_app.show_toast_message(
                f"✓ Loaded {len(proteins_dict)} proteins from FASTA", 3000
            )
            ...
            if (
                hasattr(self.main_app, "experiment_data_manager")
                and hasattr(self.main_app.experiment_data_manager, "merged_df")
                and not self.main_app.experiment_data_manager.merged_df.empty
            ):

                logger.debug("Found existing PSM data, will trigger analysis")
                self.current_psm_df = (
                    self.main_app.experiment_data_manager.merged_df.copy()
                )

                # Use QTimer to defer analysis slightly, allowing UI to update
                QTimer.singleShot(100, self.run_analysis)
            else:
                logger.debug("No PSM data available yet for automatic analysis")
    ...
    def set_psm_data(self, psm_df):
        ...
        logger.debug(
            f"protein_coverage_tab.set_psm_data called with {len(psm_df) if not psm_df.empty else 0} records"
        )
        ...
        if (
            ...
            and not self.current_psm_df.empty
            and not self.is_analyzing
        ):
            logger.debug(
                f"Triggering analysis from set_psm_data (FASTA: loaded, PSM records: {len(self.current_psm_df)})"
            )
            # Use QTimer to defer analysis slightly, preventing window flash
            QTimer.singleShot(100, self.run_analysis)
        elif self.is_analyzing:
            logger.debug("Skipping analysis trigger - already analyzing")
    ...
    def run_analysis(self):
        ...
        if not self.fasta_path:
            logger.debug("Cannot run analysis: No FASTA file loaded")
            return

        if self.current_psm_df.empty:
            logger.debug("Cannot run analysis: No PSM data available")
            return

        # If a worker is still running, queue a rerun using newest inputs.
        if self.worker is not None and self.worker.isRunning():
            logger.debug("Previous worker still running, queueing analysis rerun")
            self._analysis_queued = True
            return

        # Set analyzing flag
        self.is_analyzing = True
        logger.debug("Starting protein coverage analysis...")
        ...
        self.progress_dialog.setCancelButton(
            None
        )  # No cancel button (can't safely cancel)
    ...
    def _on_analysis_complete(self, coverage_df):
        ...
        avg_coverage = (
            coverage_df["Coverage_Percent"].mean() if not coverage_df.empty else 0
        )

        logger.debug(f"Analysis complete: {num_proteins} proteins analyzed")
        ...
        # Run one queued analysis pass if requested while worker was active.
        if (
            self._analysis_queued
            and self.fasta_path is not None
            and not self.current_psm_df.empty
        ):
            self._analysis_queued = False
    ...
    def _on_analysis_error(self, error_msg):
        ...
        logger.debug(f"Analysis error: {error_msg}")
    ...
    def _on_protein_selected(self):
        ...
        if self.coverage_df.empty:
            logger.debug("Coverage DataFrame is empty, skipping protein selection")
            return

        # Additional safety: check if we're in the middle of an analysis
        if self.is_analyzing:
            logger.debug("Analysis in progress, deferring protein selection")
            return

        protein_data = self.coverage_table.get_selected_protein_data(self.coverage_df)
        if not protein_data:
            logger.debug("No protein data found for selected row")
            return
        ...
        try:
            ...
            for key in required_keys:
                if key not in protein_data:
                    logger.debug(f"Missing required key '{key}' in protein data")
                    return
            ...
            logger.error(f"Error generating coverage HTML: {e}")
            traceback.print_exc()
        ...
        if protein_text:
            filtered_df = filtered_df[
                ...
                | filtered_df["Description"].str.contains(
                    protein_text, case=False, na=False
                )
            ]
    ...
    def cleanup(self):
        ...
        logger.debug("Cleaning up protein coverage tab resources...")
        ...
        if self.worker is not None:
            if self.worker.isRunning():
                logger.debug("Requesting stop for protein coverage worker thread")
                self.worker.requestInterruption()
                self.worker.quit()
                if not self.worker.wait(3000):
                    logger.warning(
                        "Protein coverage worker did not stop within timeout"
                    )
            self.worker.deleteLater()
            self.worker = None
        ...
        self.is_analyzing = False
        self._analysis_queued = False
        logger.debug("Protein coverage tab cleanup complete")

    def update_theme(self, theme_name):
        """Update protein coverage tab theme"""
        logger.debug(f"Updating protein coverage tab theme to {theme_name}")
        ...
        logger.debug("Protein coverage tab theme updated")
