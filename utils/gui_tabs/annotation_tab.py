...
class AnnotationTabManager:
    ...
    def setup_annotation_tab(self):
        ...
        top_layout.addWidget(
            self.graph_container, 3
        )  # Mass spec viewer gets 3/4 of space
        top_layout.addWidget(
            self.right_panel_container, 1
        )  # Analysis tools get 1/4 of space
        ...
        annotation_layout.addWidget(
            self.psm_summary_widget, 1
        )  # Bottom gets less space
        ...
        logger.debug("Annotation tab setup completed")
    ...
    def _create_analysis_tools_panel(self):
        ...
        self.right_panel_container.setMinimumWidth(
            LayoutConstants.RIGHT_PANEL_MIN_WIDTH
        )
    ...
    def _create_spectrum_tracker_widget(self, parent_layout):
        ...
        self.tracker_container = self._create_panel(widget_type="spectrum_tracker")
    ...
    def _create_peptide_info_widget(self):
        ...
        self.peptide_info_scroll.setHorizontalScrollBarPolicy(
            Qt.ScrollBarPolicy.ScrollBarAlwaysOff
        )
        self.peptide_info_scroll.setVerticalScrollBarPolicy(
    ...
    def _connect_annotation_signals(self):
        ...
        if self.mass_spec_viewer:
            ...
            # Re-run fragmentation when glycan Y-ion settings change
            if hasattr(self.mass_spec_viewer, "glycanSettingsChanged"):
                self.mass_spec_viewer.glycanSettingsChanged.connect(
                    self.main_app.event_handlers.on_settings_changed
                )

            # Connect to notify analysis tab when data changes
            if hasattr(self.mass_spec_viewer, "dataChanged"):
                self.mass_spec_viewer.dataChanged.connect(
                    self._on_mass_spec_data_changed
                )

            # Set available modifications
            if hasattr(self.main_app, "available_mods"):
                try:
                    self.mass_spec_viewer.set_available_modifications(
                        self.main_app.available_mods
                    )
                except Exception as e:
                    logger.debug(f"Error setting available modifications: {e}")

            # Pass glycan compositions list to viewer (kept for reference)
            if hasattr(self.main_app, "glycan_compositions"):
                self.mass_spec_viewer._glycan_compositions_list = (
                    self.main_app.glycan_compositions
                )
    ...
    def _on_mass_spec_data_changed(self):
        ...
        try:
            ...
            if hasattr(self.main_app, "analysis_tab_manager") and hasattr(
                self.main_app.analysis_tab_manager, "on_annotation_data_changed"
            ):
                self.main_app.analysis_tab_manager.on_annotation_data_changed()
                logger.debug("Notified analysis tab of annotation data change")

        except Exception as e:
            logger.error(f"Error notifying analysis tab of data change: {e}")
    ...
    def _on_annotation_table_view_changed(self, index):
        ...
        try:
            ...
            if (
                hasattr(self.main_app, "analysis_tab_manager")
                and hasattr(self.main_app.analysis_tab_manager, "analysis_psm_summary")
                and self.main_app.analysis_tab_manager.analysis_psm_summary
            ):
                ...
                if index == 0:  # Summary view
                    ...
                    # Sync details data
                    if (
                        hasattr(self.psm_summary_widget, "original_details_df")
                        and not self.psm_summary_widget.original_details_df.empty
                    ):

                        analysis_psm.original_details_df = (
                            self.psm_summary_widget.original_details_df.copy()
                        )
                        analysis_psm.current_details_df = (
                            self.psm_summary_widget.current_details_df.copy()
                        )
                        analysis_psm._show_details_table()

                logger.debug(
                    f"Synced annotation table view change to analysis tab: index {index}"
                )

        except Exception as e:
            logger.error(f"Error syncing annotation table view change: {e}")
    ...
    def update_peptide_info(
        ...
        if self.peptide_info_widget:
            ...
            try:
                ...
                if hasattr(self, "spectrum_tracker") and hasattr(
                    self.spectrum_tracker, "relocalise_tab"
                ):
                    self.spectrum_tracker.relocalise_tab.refresh_modifications()
            except Exception as e:
                logger.error(f"Error calculating rescore: {e}")
                rescore = None
    ...
    def calculate_rescore_for_peptide_info(self):
        ...
        try:
            ...
            filtered_data = self.filter_data_for_scoring(
                self.current_matched_data, selected_ions, include_neutral=True
            )
            ...
            needs_peptide_length = scoring_flags.get("complementary_pairs")

            peptide_length = 0
            if needs_peptide_length:
                if hasattr(self, "main_app") and hasattr(
                    self.main_app, "peptide_input"
                ):
                    try:
                ...
                if peptide_length == 0:
                    try:
                        nums = pd.to_numeric(
                            filtered_data["Ion Number"], errors="coerce"
                        ).dropna()
                        if not nums.empty:
            ...
            if scoring_flags.get("complementary_pairs"):
                # Pass all matched data so any N-term/C-term pair (y/b, y/a, c/z, etc.)
                # at a position is counted, not just the selected X!Tandem ion types.
                scores["complementary_pairs"] = self.calculate_complementary_pairs(
                    self.current_matched_data, peptide_length
                )

            return scores

        except Exception as e:
            logger.error(f"Error calculating rescore for peptide info: {e}")
            return None
    ...
    def get_selected_annotation_ion_types(self):
        ...
        try:
            ...
            if (
                hasattr(self.main_app, "selected_custom_ions_data")
                and self.main_app.selected_custom_ions_data
            ):
            ...
            if (
                hasattr(self.main_app, "selected_diagnostic_ions_data")
                and self.main_app.selected_diagnostic_ions_data
            ):
            ...
            logger.error(f"Error getting selected annotation ion types: {e}")
    ...
    def filter_data_for_scoring(self, data, ion_types, include_neutral=True):
        ...
        filtered_data = data[
            (data["Matched"].notna()) & (data["Matched"] != "No Match")
        ].copy()

        if filtered_data.empty:
            return filtered_data

        logger.debug(f"Starting with {len(filtered_data)} matched peaks")
        logger.debug(f"Ion types to include: {ion_types}")

        # Filter by selected ion types
        ion_type_mask = pd.Series(
            [False] * len(filtered_data), index=filtered_data.index
        )

        if "Ion Type" in filtered_data.columns:
            col_idx = {
                col: pos for pos, col in enumerate(filtered_data.columns, start=1)
            }
            idx_ion_type = col_idx.get("Ion Type")
            for row_tuple in filtered_data.itertuples(index=True, name=None):
                row_idx = row_tuple[0]
                ion_type_str = (
                    str(row_tuple[idx_ion_type]).strip()
                ...
                for selected_ion_type in ion_types:
                    if self._ion_type_matches_selected_for_scoring(
                        ion_type_str, selected_ion_type
                    ):
                        ion_type_mask.loc[row_idx] = True
        ...
        if not include_neutral:
            ...
            if "Neutral Loss" in filtered_data.columns:
                neutral_mask = (filtered_data["Neutral Loss"] == "None") | (
                    filtered_data["Neutral Loss"].isna()
                )
                ...
                filtered_data = filtered_data[neutral_mask]
            logger.debug(f"After neutral loss filtering: {len(filtered_data)} peaks")
    ...
    def _ion_type_matches_selected_for_scoring(self, ion_type_str, selected_ion_type):
        ...
        if selected_ion_type.startswith(("d-", "w-")):
            ...
            return (
                re.match(rf"^{base_letter}[ab]?{re.escape(loss_part)}$", ion_type_str)
                is not None
            )
    ...
    def calculate_single_xtandem(self, data, ion_types):
        """Calculate X!Tandem score using formula: HS = ln(∑Ii * Nb! * Ny!)
        - Ion counts: UNIQUE Ion Number positions per base type (isotope=0 only)
        - Intensities: all matched peak intensities summed together (excluding MH)"""
        scoring_nl_in_count = getattr(self.main_app, "scoring_nl_in_count", False)
        # Charge filter applies to X!Tandem only (not consecutive/complementary)
        scoring_max_charge = getattr(self.main_app, "scoring_max_charge", 0)
        if scoring_max_charge > 0 and not data.empty and "Charge" in data.columns:
            data = data[
                pd.to_numeric(data["Charge"], errors="coerce").fillna(1).astype(int)
                <= scoring_max_charge
            ]
        if data.empty:
            return {
        ...
        try:
            ...
            # Initialize counters
            ion_type_positions = {
                ion_type: set() for ion_type in ion_types
            ...
            if base_type_col not in data.columns:
                logger.error(f"Required column '{base_type_col}' not found")
                return self._get_empty_result(ion_types)

            if ion_number_col not in data.columns:
                logger.error(f"Required column '{ion_number_col}' not found")
                return self._get_empty_result(ion_types)

            # Process each fragment
            col_idx = {col: pos for pos, col in enumerate(data.columns, start=1)}
            idx_base_type = col_idx.get(base_type_col)
            idx_ion_number = col_idx.get(ion_number_col)
            idx_isotope = col_idx.get(isotope_col) if isotope_col else None
            idx_intensity = col_idx.get(intensity_col)
            idx_neutral_loss = col_idx.get("Neutral Loss")

            for row_tuple in data.itertuples(index=True, name=None):
                # Extract base type
                base_type = (
                    str(row_tuple[idx_base_type]).strip()
                    if idx_base_type is not None
                ...
                try:
                    ion_number_raw = (
                        row_tuple[idx_ion_number] if idx_ion_number is not None else 0
                    )
                    ion_number = int(ion_number_raw)
                except (ValueError, TypeError):
                    logger.warning(f"Invalid ion number: {ion_number_raw}")
                    continue

                # Get isotope info
                if isotope_col and isotope_col in data.columns:
                    isotope = row_tuple[idx_isotope] if idx_isotope is not None else 0
                    try:
                ...
                # Add intensity to the combined sum (all isotopes contribute)
                intensity_sum += observed_intensity
                total_intensity += observed_intensity

                # Only monoisotopic peaks count toward unique position tracking
                if isotope == 0:
                    # Skip standard NL ions from position count unless setting enabled
                    nl = (
                        str(row_tuple[idx_neutral_loss])
                        if idx_neutral_loss is not None
                        else ""
                    )
                    is_standard_nl = (
                        nl != ""
            ...
            total_ion_count = {
                ion_type: len(positions)
                for ion_type, positions in ion_type_positions.items()
            }
            ...
            for ion_type in ion_types:
                if total_ion_count[ion_type] > 0:
                    logger.debug(
                        f"{ion_type}: {total_ion_count[ion_type]} unique positions"
                    )

            # Calculate factorial product using unique monoisotopic position counts
            factorial_product = 1
            ...
            for ion_type in ion_types:
                ...
                if unique_count > 0:
                    ...
                    logger.debug(f"{ion_type}! = {unique_count}! = {factorial_val}")

            logger.debug(f"Total factorial product: {factorial_product}")
            logger.debug(f"Total intensity sum: {intensity_sum:.2f}")

            # Calculate final X!Tandem score: HS = ln(∑Ii * Nb! * Ny!)
            if intensity_sum > 0 and factorial_product > 0:
                xtandem_raw = intensity_sum * factorial_product
                xtandem = math.log(xtandem_raw)
                logger.debug(
                    f"X!Tandem: ln({intensity_sum:.2f} * {factorial_product}) = {xtandem:.4f}"
                )
            else:
                xtandem = 0.0
            ...
            logger.error(f"Error calculating X!Tandem score: {e}")
    ...
    def calculate_consecutive_ion_series(self, matched_data):
        ...
        try:
            ...
            if (
                mono.empty
                or "Base Type" not in mono.columns
                or "Ion Number" not in mono.columns
            ):
                return result
            ...
            logger.error(f"Consecutive ion series calculation error: {e}")
            return result
    ...
    def calculate_complementary_pairs(self, matched_data, peptide_length):
        ...
        try:
            ...
            col_idx = {col: pos for pos, col in enumerate(mono.columns, start=1)}
            idx_base_type = col_idx.get("Base Type")
            idx_ion_number = col_idx.get("Ion Number")
            idx_ion_type = col_idx.get("Ion Type")

            for row_tuple in mono.itertuples(index=True, name=None):
                base_type = (
                    str(row_tuple[idx_base_type]).strip()
                    if idx_base_type is not None
                    else ""
                ...
                ion_type_full = (
                    str(row_tuple[idx_ion_type]).lower()
                    if idx_ion_type is not None
                    else ""
                )
            ...
            logger.error(f"Complementary pairs calculation error: {e}")
            return empty
    ...
    def _update_mass_spectrum_viewer_theme(self, theme_name):
        ...
        try:
            ...
            if hasattr(self.mass_spec_viewer, "spectrumplot"):
                self.mass_spec_viewer.spectrumplot.getViewBox().setBackgroundColor(
                    plot_bg_color
                )

            # Update error plot background
            if hasattr(self.mass_spec_viewer, "errorbarplot"):
                self.mass_spec_viewer.errorbarplot.getViewBox().setBackgroundColor(
                    plot_bg_color
                )
            ...
            if (
                hasattr(self.mass_spec_viewer, "df")
                and not self.mass_spec_viewer.df.empty
            ):
            ...
            if (
                hasattr(self.mass_spec_viewer, "peptide_sequence")
                and self.mass_spec_viewer.peptide_sequence
            ):
                self.mass_spec_viewer._draw_peptide_sequence()
                # Restore modification display if any
                if (
                    hasattr(self.mass_spec_viewer, "current_modifications")
                    and self.mass_spec_viewer.current_modifications
                ):
                    self.mass_spec_viewer.update_modification_display()

        except Exception as e:
            logger.debug(f"Error updating mass spectrum viewer theme: {e}")
    ...
    def _update_psm_summary_theme(self):
        ...
        if hasattr(self.psm_summary_widget, "summary_filter_widget"):
            self.psm_summary_widget.summary_filter_widget.setStyleSheet(
                filter_widget_style
            )
        if hasattr(self.psm_summary_widget, "details_filter_widget"):
            self.psm_summary_widget.details_filter_widget.setStyleSheet(
                filter_widget_style
            )
