...
from utils.utilities import DataLoader, FileProcessingUtils, UIHelpers, FileTypeUtils
from utils.spectral_extraction.batch_spectral_extraction import (
...
class ExperimentDataManager:
    def __init__(self, main_app):
        ...
        self.scan_counts = {}  # {raw_file_path: {'ms1': N, 'ms2': M, 'total': T}}
    ...
    def load_raw_data(self):
        ...
        if files:
            valid_files, invalid_files = (
                FileProcessingUtils.validate_and_load_raw_files(files)
            )
    ...
    def add_msfragger_search_folder(self):
        ...
        # Use utility function instead of manual processing
        matched_psm_files, invalid_files = FileProcessingUtils.process_msfragger_folder(
            folder
        )
    ...
    def _create_and_save_dataframe(self):
        ...
        logger.debug("File paths CSV updated:\n%s", self.df_file_paths)
    ...
    def combine_and_process_psm_files(self):
        ...
        from utils.mod_database import (
        ...
        if not self.search_files:
            logger.debug("No search files loaded.")
            return
        ...
        if unrecognised_paths:
            from utils.psm_normalizers.psm_utils_format_dialog import (
            ...
            if fmt_dialog.exec() == _QDialog.DialogCode.Accepted:
                selections = (
                    fmt_dialog.get_selections()
                )  # {basename: format_key | None}
                for path in unrecognised_paths:
                    chosen = selections.get(os.path.basename(path))
                    if chosen:
                        file_groups.setdefault(chosen, []).append(path)
                        logger.info(
                            f"User assigned '{chosen}' to {os.path.basename(path)}"
                        )
                    else:
                        logger.warning(
                            f"Skipping {os.path.basename(path)} (no format selected)"
                        )
            else:
                # User cancelled the dialog — skip all unrecognised files
                logger.warning(
                    f"Format dialog cancelled; {len(unrecognised_paths)} file(s) skipped."
                )
        ...
        logger.debug(f"Detected engines: {engines_used}  ({total_files} files)")
        ...
        # Pre-create psm_utils normalizer instances keyed by file path so they
        # can be reused across Steps 3a, 3b, and 5.  This lets the PSM list
        # and ptm_result cache built during pre-scan be reused during normalize,
        # avoiding multiple full XML parses of the same file.
        _psm_utils_norms: dict = {}
        for _engine, _paths in file_groups.items():
            if NormalizerFactory.is_psm_utils_format(_engine):
                for _p in _paths:
                    _psm_utils_norms[_p] = NormalizerFactory.create(
                        _engine, source_file_path=_p
                    )
        ...
        for engine, paths in file_groups.items():
            if engine not in (
                ...
                "MetaMorpheus",
                "Byonic",
            ) and not NormalizerFactory.is_psm_utils_format(engine):
                continue
            if engine == "MaxQuant":
                db = maxquant_db
            ...
            normalizer = NormalizerFactory.create(
                engine, mod_database=db, source_file_path=paths[0] if paths else None
            )
            ...
            for path in paths:
                try:
                    if NormalizerFactory.is_psm_utils_format(engine):
                        # Reuse the pre-created normalizer so the PSM read is cached
                        normalizer_path = _psm_utils_norms.get(path) or NormalizerFactory.create(
                    ...
                    logger.warning(f"Could not pre-scan {path}: {e}")
        ...
        # ---------------------------------------------------------------
        # Step 3b: Pre-scan for unlocalized ptm_result masses (pepXML)
        # ---------------------------------------------------------------
        ptm_localization_prefs: dict[float, str | None] = {}
        for engine, paths in file_groups.items():
            if engine != "pepXML":
                continue
            all_unlocalized: dict[float, int] = {}
            for path in paths:
                try:
                    temp_norm = _psm_utils_norms.get(path) or NormalizerFactory.create(
                        engine, source_file_path=path
                    )
                    unlocalized = temp_norm.pre_scan_ptm_results()
                    for mass_val, count in unlocalized.items():
                        all_unlocalized[mass_val] = (
                            all_unlocalized.get(mass_val, 0) + count
                        )
                except Exception as e:
                    logger.warning(f"Could not pre-scan ptm_results in {path}: {e}")

            if all_unlocalized:
                from utils.psm_normalizers.ptm_result_dialog import (
                    PtmResultLocalizationDialog,
                )

                ptm_dialog = PtmResultLocalizationDialog(
                    all_unlocalized, parent=self.main_app
                )
                if ptm_dialog.exec() == QDialog.DialogCode.Accepted:
                    ptm_localization_prefs = ptm_dialog.get_preferences()
                else:
                    # User cancelled — ptm_result data will be ignored
                    logger.warning(
                        "PTM localization dialog cancelled; ptm_result data will be ignored for all files"
                    )
        ...
        try:
            ...
            for engine, paths in file_groups.items():
                db = (
                    maxquant_db
                    if engine == "MaxQuant"
                    else metamorpheus_db if engine == "MetaMorpheus" else None
                )

                for path in paths:
                    if progress.wasCanceled():
                        logger.debug("User canceled operation.")
                        return
                    ...
                    try:
                        # Reuse pre-created normalizer for psm_utils formats so the
                        # PSM-list cache and ptm_result cache from pre-scan are preserved
                        if NormalizerFactory.is_psm_utils_format(engine):
                            normalizer = _psm_utils_norms.get(path) or NormalizerFactory.create(
                        ...
                        # Pass ptm_result localization prefs to pepXML normalizer
                        if engine == "pepXML" and ptm_localization_prefs:
                            normalizer.set_ptm_localization_prefs(
                                ptm_localization_prefs
                            )
                        ...
                        # Free the PSM cache immediately after normalization
                        if hasattr(normalizer, "clear_psm_cache"):
                            normalizer.clear_psm_cache()
                        all_normalised.append(df_norm)
                        logger.debug(
                            f"Normalised {os.path.basename(path)}: {len(df_norm)} rows ({engine})"
                        )
                    except Exception as e:
            ...
            if not all_normalised:
                logger.debug("No valid PSM data to combine.")
                progress.close()
                return

            combined_df = pd.concat(all_normalised, ignore_index=True)
            logger.debug(
                f"Combined dataframe has {len(combined_df)} rows before duplicate removal"
            )
            ...
            if all(c in combined_df.columns for c in dup_cols):
                ...
                logger.debug(f"Removed {duplicates_removed} duplicate PSMs")
                ...
                logger.warning("Cannot remove duplicates – missing columns")
            ...
            df_file_paths["raw_file_base"] = df_file_paths["Raw data file name"].apply(
                FileTypeUtils.strip_file_extension
            )
            ...
            merged_df["index"] = merged_df["index"].astype(str).str.lstrip("0")
            ...
            progress.setLabelText("Extracting spectral data from raw files...")
            QApplication.processEvents()

            self._extract_all_spectral_data(progress)

            # -----------------------------------------------------------
            # Step 8b: Count MS1/MS2 scans
            # -----------------------------------------------------------
            self._count_all_scans(progress)
            ...
            # Update Results tab
            if hasattr(self.main_app, "results_tab_manager"):
                self.main_app.results_tab_manager.update_results_table()

            logger.debug("Combine & Process PSM done.")
            ...
            final_message += (
                "Extracted spectral data for analysis.\n\n"
                "Would you like to save this as an experiment file "
                "for quick reloading?"
            )
            ...
            logger.error(f"Error in combine_and_process_psm_files: {e}")
            traceback.print_exc()
            if progress:
                progress.close()
            UIHelpers.show_validation_error(
    ...
    def _extract_all_spectral_data(self, progress=None):
        ...
        logger.debug("Starting optimized batch spectral data extraction...")
        ...
        if not all(col in self.merged_df.columns for col in required_columns):
            logger.debug(f"Missing required columns for extraction: {required_columns}")
            if progress:
        ...
        if valid_rows.empty:
            logger.debug("No valid rows found for extraction")
            if progress:
                progress.setValue(progress.maximum())
            return

        logger.debug(f"Extracting spectral data for {len(valid_rows)} PSMs...")
        ...
        try:
            ...
            results_df = ultra_fast_extract_lightweight(input_file=temp_input)
            ...
            logger.error(f"Error during optimized batch extraction: {e}")
            traceback.print_exc()
    ...
    def _merge_extracted_data(self, extracted_df):
        ...
        logger.debug("Starting optimized data merging with header information...")
        ...
        if successful_rows.empty:
            logger.debug("No successful extractions to merge")
            return
        ...
        logger.debug(
            f"Optimized merging with headers completed for {len(grouped_data)} unique scans in {elapsed_time:.2f}s"
        )
    ...
    def _add_header_column_to_merged_df(self, extracted_df):
        ...
        try:
            ...
            def _normalize_scan_component(value):
                if pd.isna(value):
                    return ""
            ...
            for row in extracted_df.itertuples(index=False):
                file_path = row.file_path
                scan_number = _normalize_scan_component(row.scan_number)
            ...
            spectrum_series = (
                self.merged_df.get(
                    "spectrum_file_path", pd.Series("", index=self.merged_df.index)
                )
                .fillna("")
                .astype(str)
            )
            index_series = self.merged_df.get(
                "index", pd.Series("", index=self.merged_df.index)
            ).map(_normalize_scan_component)
            cache_keys = spectrum_series + "_" + index_series

            # Vectorized map instead of row-wise apply for better performance on large tables
            self.merged_df["Header"] = cache_keys.map(header_mapping)

            logger.debug(
                f"Added header information to {len(self.merged_df)} rows in merged_df"
            )

        except Exception as e:
            logger.error(f"Error adding header column: {e}")

    def _count_all_scans(self, progress=None):
        """Count MS1/MS2 scans for all loaded raw files."""
        from utils.spectral_extraction.scan_counter import count_scans_batch

        # Collect raw file paths that need counting
        file_paths = [fp for fp in self.raw_files if fp not in self.scan_counts]
        if not file_paths:
            return

        if progress:
            progress.setLabelText("Counting MS1/MS2 scans from raw files...")
            QApplication.processEvents()

        try:
            new_counts = count_scans_batch(file_paths, max_workers=4)
            self.scan_counts.update(new_counts)
            for fp, counts in new_counts.items():
                logger.debug(
                    f"Scan counts for {os.path.basename(fp)}: MS1={counts['ms1']}, MS2={counts['ms2']}, Total={counts['total']}"
                )
        except Exception as e:
    ...
    def save_experiment(self):
        ...
        if (
            has_rescoring
            and hasattr(self, "_last_experiment_path")
            and self._last_experiment_path
        ):
    ...
    Choose 'No' to save as a new file instead."""
        ...
        if not file_path:
            default_name = (
                "experiment_rescored.kcx" if has_rescoring else "experiment.kcx"
            )
    ...
    Search Files: {metadata['search_files_count']}"""
                ...
                progress.close()
                UIHelpers.show_validation_error(
                    self.main_app, "Save Error", "Failed to save experiment"
                )
    ...
    def _save_experiment_data(self, experiment_data, file_path):
        ...
        try:
            ...
            logger.debug(f"Experiment saved successfully to {file_path}")
            return True
    ...
    def load_experiment(self):
        ...
        file_path, _ = (
            QFileDialog.getOpenFileName(  # CHANGED: Removed 's' from getOpenFileNames
                self.main_app,
        ...
        try:
            ...
            if not success:
                progress.close()
                UIHelpers.show_validation_error(
                    self.main_app, "Invalid File", error_msg
                )
                return
            ...
            self.df_file_paths = pd.DataFrame(
                experiment_data["file_paths"]["df_file_paths"]
            )
            ...
            self.extracted_spectral_data = experiment_data.get(
                "extracted_spectral_data", {}
            )
            self.scan_counts = experiment_data.get("scan_counts", {})
            ...
            self.main_app.annotation_tab_manager.set_data(
                self.merged_df
            )  # Annotation tab
            self.main_app.frag_psm_summary_widget.setData(
                self.merged_df
            )  # Fragmentation tab
            ...
            if "rescored_df" in experiment_data and experiment_data["rescored_df"]:
                ...
                self.rescoring_metadata = experiment_data.get(
                    "rescoring_metadata", None
                )

                # Load into rescoring tab viewer
                if (
                    hasattr(self.main_app, "rescoring_tab_manager")
                    and self.main_app.rescoring_tab_manager.results_viewer
                ):
                    self.main_app.rescoring_tab_manager.load_existing_rescoring(
                        self.rescored_df
                    )

                logger.debug(f"Loaded rescoring data: {len(self.rescored_df)} PSMs")
            else:
                self.rescored_df = None
                self.rescoring_metadata = None
                logger.debug("No rescoring data in experiment file")
        ...
        try:
            ...
            if missing_keys:
                return (
                    False,
                    None,
                    f"Invalid experiment file. Missing keys: {missing_keys}",
                )

            logger.debug(f"Experiment loaded successfully from {file_path}")
    ...
    def _create_experiment_data(self):
        ...
        unique_peptides = (
            len(self.merged_df["Peptide"].unique()) if not self.merged_df.empty else 0
        )
        ...
        if self.rescored_df is not None:
            ...
            logger.debug(f"Including rescoring data: {len(self.rescored_df)} PSMs")
    ...
    def save_rescoring_to_experiment(self, rescored_df, rescoring_settings):
        ...
        logger.debug(f"Rescoring data stored: {len(rescored_df)} PSMs")
    ...
    def _restore_experiment_settings(self, experiment_data):
        ...
        self.main_app.text_annotation_threshold.setValue(
            settings.get("text_annotation_threshold", 0)
        )
    ...
    def update_spectrum_quality_in_dataframe(self, spectrum_data, quality):
        ...
        try:
            ...
            logger.debug(f"Updated {mask.sum()} rows with quality: {quality}")

        except Exception as e:
            logger.error(f"Failed to update quality in dataframe: {e}")
    ...
    def _register_modifications_in_central_db(self, df, central_db):
        ...
        if seen_masses:
            ...
            logger.debug(f"Registered {len(seen_masses)} unique modification masses")

    def extract_scan_data(
        self, selected_file: str, scan_number: str
        ...
        if not selected_file or not scan_number:
            logger.error("Missing file or scan number")
            return None

        try:
            scan_num = int(scan_number)
        except ValueError:
            logger.error(f"Invalid scan number: {scan_number}")
            return None
        ...
        if not file_path:
            logger.error(f"Could not find raw file: {selected_file}")
            return None

        logger.debug(f"Extracting scan {scan_num} from {file_path}")

        try:

            # Extract the single scan
            results = extract_multiple_scans_from_file_lightweight(
                file_path, [scan_num]
            )

            if not results:
                logger.error(f"No results returned for scan {scan_num}")
                return None

            result = results[0]  # Get the first (and only) result

            if result["status"] != "success":
                logger.error(f"Scan extraction failed: {result['status']}")
                UIHelpers.show_validation_error(
                    self.main_app,
                    "Extraction Error",
            ...
            if len(mz_list) == 0 or len(intensity_list) == 0:
                logger.error(f"Empty data returned for scan {scan_num}")
                return None

            logger.debug(
                f"Successfully extracted {len(mz_list)} peaks from scan {scan_num}"
            )
            UIHelpers.show_success_message(
                self.main_app, f"Extracted {len(mz_list)} peaks from scan {scan_num}"
            )

            return mz_list, intensity_list

        except Exception as e:
            logger.error(f"Exception during scan extraction: {e}")
