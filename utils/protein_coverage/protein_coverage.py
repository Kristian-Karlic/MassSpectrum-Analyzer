...
class ProteinCoverageAnalyzer:
    ...
    def analyze_psm_data(self, psm_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for protein_accession, group in grouped:
            ...
            # Drop duplicates on 'Peptide' only — 'Parsed Modifications' may contain
            # lists which are unhashable and incompatible with drop_duplicates()
            cols = ["Peptide", "Assigned Modifications", "Peptide Length"]
            if "Parsed Modifications" in group.columns:
                cols.append("Parsed Modifications")
            unique_peptides_data = group[cols].drop_duplicates(subset=["Peptide"])
            unique_peptides_data = unique_peptides_data[
                unique_peptides_data["Peptide"].notna()
            ]
            ...
            peptide_mappings = []
            modification_sites = defaultdict(set)  # {protein_position: {mod_masses}}

            col_idx = {
                col: pos
                for pos, col in enumerate(unique_peptides_data.columns, start=1)
            }
            idx_peptide = col_idx.get("Peptide")
            idx_parsed_mods = col_idx.get("Parsed Modifications")
            idx_peptide_length = col_idx.get("Peptide Length")

            for row_tuple in unique_peptides_data.itertuples(index=False, name=None):
                peptide = row_tuple[idx_peptide - 1] if idx_peptide is not None else ""
                parsed_mods = (
                    row_tuple[idx_parsed_mods - 1]
                    if idx_parsed_mods is not None
                    else None
                )
                peptide_length = (
                    row_tuple[idx_peptide_length - 1]
                    if idx_peptide_length is not None
                    else len(peptide)
                )
                peptide_length = (
                    int(peptide_length) if peptide_length is not None else len(peptide)
                )
                ...
                if positions:
                    ...
                    for start_pos in positions:
                        ...
                        for peptide_pos, mod_mass in peptide_mods:
                            ...
                            modification_sites[protein_pos].add(mod_mass)
                else:
                    print(
                        f"Warning: Peptide '{peptide}' not found in protein '{protein_accession}'"
                    )
            ...
            modification_sites_output = {
                protein_pos: sorted(mod_masses)
                for protein_pos, mod_masses in modification_sites.items()
            }
            ...
            results.append(
                {
                    ...
                    "Modification_Sites": modification_sites_output,
                }
            )
...
class CoverageHTMLGenerator:
    ...
    @staticmethod
    def assign_modification_colors(
        modification_sites: Dict[int, List[float]],
    ) -> Dict[float, str]:
        """
    ...
    def generate_coverage_html(
        ...
        modification_sites: Dict[int, List[float]] = None,
    ) -> str:
        """
        ...
        mod_color_map = CoverageHTMLGenerator.assign_modification_colors(
            modification_sites
        )
        html_parts = []
