...
def _match_core(
    ...
    ----------
    mode : str
        ``"dict"`` when ions are dicts (match_fragment_ions); ``"tuple"`` when ions
        are tuples (match_fragment_ions_fast).  Controls how ion fields are read.
    ...
    all_candidates_per_peak = (
        [[] for _ in range(num_user)] if track_alternatives else None
    )
    ...
    for u_idx in range(num_user):
        ...
        candidate_orig_indices = sort_idx[left_idx:right_idx]
        ...
        candidate_orig_indices = [
            c for c in candidate_orig_indices if c not in matched_ions
        ]
        ...
        # Fast-path: single candidate, skip sort
        if len(candidate_orig_indices) == 1:
            sort_c = [0]
        else:
        ...
        for idx_c in sort_c:
            ...
            if mode == "dict":
                iso = ion_info["Isotope"]
                ion_num = ion_info["Ion Number"]
                ion_type = ion_info["Ion Type"]
...
def match_fragment_ions(calculated_ions, user_mz_values, ppm_tolerance=10):
    ...
    """
    _mz_arr = np.array(user_mz_values, dtype=np.float64)
    user_mzs = _mz_arr[:, 0]
    user_intensities = _mz_arr[:, 1]

    if isinstance(calculated_ions, pd.DataFrame):
        all_ions = calculated_ions.to_dict(orient="records")
        theo_masses_arr = calculated_ions["Theoretical Mass"].to_numpy(dtype=np.float64)
    else:
        all_ions = calculated_ions
        theo_masses_arr = np.array(
            [ion["Theoretical Mass"] for ion in all_ions], dtype=np.float64
        )
    ...
    for u_idx in range(num_user):
        ...
        ion_data = {
            ...
            "Color": None,
            "Base Type": None,
            "Ion Series Type": None,
            "Modified Fragment": None,
        }

        if matches[u_idx]:
            i_idx = matched_indices[u_idx]
            err_ppm = ppm_errors[u_idx]
            ion_info = all_ions[i_idx]
            ...
            ion_data = {
                ...
                "Ion Series Type": ion_info.get(
                    "Ion Series Type", "Standard-Ion-Series"
                ),
                "Modified Fragment": ion_info.get("Modified Fragment"),
            }
...
def match_fragment_ions_fast(
    theoretical_tuples, user_mz_values, ppm_tolerance=10, diagnostic_ions=None
):
    """
    ...
    Args:
        theoretical_tuples: List of tuples (Theoretical Mass, Ion Number, Ion Type,
                           Fragment Sequence, Neutral Loss, Charge, Isotope, Color,
                           Base Type, Modified Fragment)
        user_mz_values: List of (mz, intensity) tuples
        ppm_tolerance: PPM tolerance for matching
        diagnostic_ions: Optional list of (name, mass, color) diagnostic ions

    Returns:
        List of 13-element tuples: (m/z, intensity, Matched, error_ppm, Ion Number,
                                     Ion Type, Fragment Sequence, Neutral Loss, Charge,
                                     Isotope, Color, Base Type, Modified Fragment)
    """
    _mz_arr = np.array(user_mz_values, dtype=np.float64)
    user_mzs = _mz_arr[:, 0]
    user_intensities = _mz_arr[:, 1]
    ...
    # Tuple indices: 0=Mass, 1=IonNum, 2=IonType, 3=FragSeq, 4=NeutLoss, 5=Charge,
    #                6=Isotope, 7=Color, 8=BaseType, 9=ModifiedFragment
    if diagnostic_ions:
        all_ions = list(theoretical_tuples)
        for ion_name, mass_val, color in diagnostic_ions:
            all_ions.append(
                (mass_val, "", ion_name, "", "Custom_Ion", 1, 0, color, None, None)
            )
    else:
        all_ions = theoretical_tuples
        ...
        if matches[u_idx]:
            ...
            result_tuples.append(
                (
                    mz_val,
                    intensity_val,
                    ion[0],  # Matched = theoretical mass
                    ppm_errors[u_idx],
                    ion[1],
                    ion[2],
                    ion[3],
...
def fragment_and_match_peaks_cached(
    peptide,
    modifications,
    max_charge,
    ppm_tolerance,
    ...
    try:
        ...
        if diagnostic_ions:
            ...
            calculated_ions = pd.concat(
                [calculated_ions, df_diagnostic], ignore_index=True
            )

        # Store theoretical data
        theoretical_data = calculated_ions

        # Step 4: Match with experimental data
        matched_data = match_fragment_ions(
            calculated_ions,  # pass DataFrame directly — no to_dict()
            user_mz_values,
            ppm_tolerance,
        )
